        ...
        prev.map((msg) =>
          msg.id === aiMsgId
            ? {
                ...msg,
                isLoading: false,
                error: err?.message || "An unexpected error occurred while drafting the proposal.",
              }
            : msg
        )
      );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-xl tracking-tight text-white flex items-center gap-2">
                AgentDAO <span className="text-xs px-2 py-0.5 rounded-full bg-[#7B4FF2]/20 border border-[#7B4FF2]/40 text-[#00E5C7] font-mono">v0.1</span>
              </h1>
              <p className="text-xs text-[#9E9BB9]">AI-Powered Governance Proposal Generator</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6 flex flex-col">
        ...
        <div className="flex-1 space-y-6 mb-6">
          {messages.map((msg) => (
            ...
            >
              ...
              {/* Message Content */}
              <div className={`max-w-2xl w-full ${msg.sender === "user" ? "flex flex-col items-end" : ""}`}>
                <div className="flex items-center gap-2 mb-1 px-1">
                  <span className="text-xs font-medium text-[#9E9BB9]">
                    {msg.sender === "user" ? "You" : "AgentDAO AI"}
                  </span>
                  <span className="text-[10px] text-[#9E9BB9]/60">{msg.timestamp}</span>
                </div>
                ...
                {msg.error && (
                  <div className="bg-red-950/40 border border-red-500/30 text-red-200 rounded-2xl rounded-tl-none p-4 flex items-start gap-3">
                    ...
                    <div className="text-sm">
                      <p className="font-semibold text-red-300">Drafting Failed</p>
                      <p className="text-xs text-red-300/80 mt-1">{msg.error}</p>
                    </div>
                  </div>
                )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#2E265C]">
                      ...
                      <div className="bg-[#12102B] px-4 py-2 rounded-xl border border-[#2E265C] flex items-center gap-2 self-start sm:self-auto shrink-0 shadow-inner">
                        ...
                        <div className="text-right">
                          <div className="text-[10px] uppercase text-[#9E9BB9] font-mono">Requested</div>
                    ...
                    <div className="space-y-4 text-sm">
                      ...
                      <div>
                        <h3 className="text-xs uppercase font-mono text-[#9E9BB9] tracking-wider mb-1 flex items-center gap-1.5">
                          <FileText className="w-3.5 h-3.5 text-[#7B4FF2]" /> Executive Summary
                        </h3>
                      ...
                      <div>
                        <h3 className="text-xs uppercase font-mono text-[#9E9BB9] tracking-wider mb-1 flex items-center gap-1.5">
                          <CheckCircle2 className="w-3.5 h-3.5 text-[#00E5C7]" /> Rationale & ROI
                        </h3>
                        <p className="text-[#F1F0FB]/90 leading-relaxed bg-[#12102B]/50 p-3.5 rounded-xl border border-[#2E265C]/60">
        ...
        <div className="mb-4">
          <p className="text-xs text-[#9E9BB9] mb-2 font-mono flex items-center gap-1">
            <Sparkles className="w-3.5 h-3.5 text-[#7B4FF2]" /> Try an example request:
          </p>
